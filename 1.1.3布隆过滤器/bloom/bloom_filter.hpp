...
public:
   ...
   bloom_filter()
   : salt_count_(0),    // 盐值，不能超过128个
     table_size_(0),    // 位图的长度
     projected_element_count_(0),   // 预期插入的元素的数量
     inserted_element_count_ (0),   // 现在已经插入的元素的数量
     random_seed_(0),               // 随机值
     desired_false_positive_probability_(0.0)   // 假阳率
   {}
   ...
   inline void insert(const unsigned char* key_begin, const std::size_t& length)
   {
      ...
      for (std::size_t i = 0; i < salt_.size(); ++i)
      {
         ...
         bit_table_[bit_index / bits_per_char] |= bit_mask[bit];  // 把位图中对应位置1
      }
   ...
   inline void insert(const T& t)
   {
      ...
      // 插入这边使用了模板，只要插入时key可以转换为const unsigned char*这种类型，就调用这个函数
